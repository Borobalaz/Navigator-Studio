...
import { type CegRecord as CegRecordType } from '../db';

export type ExcelLikeRow = {
  indent: number;
  field: string;
  value?: string;
  kind?: 'section' | 'field';
...
export const KIEMELT_ADATOK_FIELDS: Array<keyof CegRecordType> = [
  'Megnevezése',
  'Adószáma',
  'Vámazonosító száma',
  'Közösségi adószáma',
...
export function buildExcelLikeRows(company: CegRecord, data: CompanyViewData): ExcelLikeRow[] {
...
export function buildEmployeeRows(employees: MunkavallaloRecord[]): ExcelLikeRow[] {
  const rows: ExcelLikeRow[] = [];
...
export function buildKepviseloRows(kepviselok: KepviseloRecord[]): ExcelLikeRow[] {
  const rows: ExcelLikeRow[] = [];

  pushSection(rows, 0, 'Képviselők');

  if (kepviselok.length === 0) {
    pushField(rows, 1, 'Státusz', 'Nincs képviselő a kiválasztott céghez.');
    return rows;
  }

  kepviselok.forEach((kepviselo, index) => {
    pushSection(rows, 1, `${index + 1}. képviselő`);
    pushField(rows, 2, 'Név/elnevezés', kepviselo['név/elnevezés']);
    pushField(rows, 2, 'Adóazonosító jel/adószám', kepviselo['adóazonosító jel/adószám']);
    pushField(rows, 2, 'Képviselet jellege', kepviselo['képviselet jellege (törvényes képviselő, szervezeti képviselő, felszámoló, végelszámoló)']);
    pushField(rows, 2, 'Képviselet terjedelme', kepviselo['képviselet terjedelme (önálló, együttes, képviseleti jogot nem gyakorol)']);
    pushField(rows, 2, 'Képviseleti jog kezdete', kepviselo['képviseleti jog kezdete']);
  });

  return rows;
}

export function buildTulajdonosRows(tulajdonosok: TulajdonosRecord[]): ExcelLikeRow[] {
  const rows: ExcelLikeRow[] = [];

  pushSection(rows, 0, 'Tulajdonosok');

  if (tulajdonosok.length === 0) {
    pushField(rows, 1, 'Státusz', 'Nincs tulajdonos a kiválasztott céghez.');
    return rows;
  }

  tulajdonosok.forEach((tulajdonos, index) => {
    pushSection(rows, 1, `${index + 1}. tulajdonos`);
    pushField(rows, 2, 'Neve', tulajdonos['Neve']);
    pushField(rows, 2, 'Adóazonosító jele', tulajdonos['Adóazonosító jele']);
    pushField(rows, 2, 'Lakóhelye', tulajdonos['Lakóhelye']);
    pushField(rows, 2, 'Érvényesség kezdete', tulajdonos['Érvényesség kezdete']);
  });

  return rows;
}

export function buildBankszamlaRows(bankszamlak: BankszamlaRecord[]): ExcelLikeRow[] {
  const rows: ExcelLikeRow[] = [];

  pushSection(rows, 0, 'Bankszámlaszámok');

  if (bankszamlak.length === 0) {
    pushField(rows, 1, 'Státusz', 'Nincs bankszámla a kiválasztott céghez.');
    return rows;
  }

  bankszamlak.forEach((bankszamla, index) => {
    pushSection(rows, 1, `${index + 1}. bankszámla`);
    pushField(rows, 2, 'Számla száma', bankszamla['számla száma']);
    pushField(rows, 2, 'Érvényesség kezdése', bankszamla['érvényesség kezdése']);
  });
