/**
 * FEOR (Foglalkozási Egyeztetett Objektum Rendszer) - Hungarian Classification of Occupations
* Official 4-digit FEOR 08 codes with profession names
* Source: Hungarian Central Statistical Office (KSH)
*/

export const FEOR_CODES: Record<string, string> = {
  ...
  // 23 Szociális szolgáltatási foglalkozások
  ...
  // 27 Kulturális, sport-, művészeti és vallási foglalkozások
  ...
  '4226': 'Járásikörzeti kérdező, összeíró',
  ...
  '7232': 'Nyomdász, nyomdaipari gépmester',
  '7233': 'Könyvkötő',
...
/**
 * Get the profession name for a FEOR code
 * @param feorCode The FEOR code to look up
 * @returns The profession name, or the original code if not found
 */
export function getFEORName(feorCode: string | undefined): string {
  if (!feorCode) return '';
  const trimmed = feorCode.trim();
  return FEOR_CODES[trimmed] || trimmed;
}

/**
 * Search FEOR codes by name or code
 * @param query Search query (partial code or profession name)
 * @returns Array of matching FEOR codes with their names
 */
export function searchFEORCodes(query: string): Array<{ code: string; name: string }> {
  const lowerQuery = query.toLowerCase();
  return Object.entries(FEOR_CODES)
    .filter(
      ([code, name]) =>
        code.toLowerCase().includes(lowerQuery) || name.toLowerCase().includes(lowerQuery),
    )
    .map(([code, name]) => ({ code, name }));
}

/**
 * Get all available FEOR codes
 * @returns Array of all FEOR codes with their names
 */
export function getAllFEORCodes(): Array<{ code: string; name: string }> {
  return Object.entries(FEOR_CODES).map(([code, name]) => ({ code, name }));
}
