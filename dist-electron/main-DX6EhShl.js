import require$$1$4, { app, BrowserWindow, ipcMain, shell, dialog } from "electron";
...
if (!fs$h[gracefulQueue]) {
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || "")) {
    process.on("exit", function() {
      ...
      require$$0$4.equal(fs$h[gracefulQueue].length, 0);
    });
  }
}
...
var makeDir$1 = {};
var utils$2 = {};
const path$l = require$$1__default;
...
const fs$d = fs$i;
const path$k = require$$1__default;
...
const path$j = require$$1__default;
...
const path$i = require$$1__default;
...
const fs_1$5 = fs__default;
const stream_1$2 = require$$0$3;
const url_1$7 = require$$2$3;
...
var exception$1 = YAMLException$4;
...
jsYaml.YAMLException = exception$1;
jsYaml.types = {
  binary,
  float,
  map,
...
const fs_1$2 = fs__default;
const DataSplitter_1 = DataSplitter$1;
const url_1$1 = require$$2$3;
...
function extend2(obj) {
  for (var i = 1; i < arguments.length; i++) {
    for (var key2 in arguments[i]) {
      if (Object.prototype.hasOwnProperty.call(arguments[i], key2)) {
        obj[key2] = arguments[i][key2];
      }
    }
  }
  return obj;
...
var _exception2$4 = _interopRequireDefault$5(_exception$4);
...
(function(module, exports$1) {
  ...
  Visitor.prototype = {
    ...
    acceptKey: function acceptKey(node2, name) {
      ...
      if (this.mutating) {
        if (value && !Visitor.prototype[value.type]) {
          throw new _exception22["default"]('Unexpected node type "' + value.type + '" found when accepting ' + name + " on " + node2.type);
        }
        node2[name] = value;
      }
    },
    ...
    acceptRequired: function acceptRequired(node2, name) {
      ...
      if (!node2[name]) {
        throw new _exception22["default"](node2.type + " requires " + name);
      }
    },
...
(function(module, exports$1) {
  ...
  var _visitor3 = visitorExports;
  var _visitor22 = _interopRequireDefault2(_visitor3);
  function WhitespaceControl() {
    var options = arguments.length <= 0 || arguments[0] === void 0 ? {} : arguments[0];
    this.options = options;
  }
  WhitespaceControl.prototype = new _visitor22["default"]();
...
function preparePath(data, parts, loc) {
  ...
  for (var i = 0, l = parts.length; i < l; i++) {
    ...
    if (!isLiteral && (part === ".." || part === "." || part === "this")) {
      if (dig.length > 0) {
        throw new _exception2$2["default"]("Invalid path: " + original, { loc });
      } else if (part === "..") {
        depth++;
      }
    } else {
...
function validateInputAst(ast2) {
  validateAstNode(ast2);
}
  ...
  CodeGen.prototype = {
    ...
    objectLiteral: function objectLiteral(obj) {
      ...
      Object.keys(obj).forEach(function(key2) {
        var value = castChunk(obj[key2], _this);
        if (value !== "undefined") {
          pairs2.push([_this.quotedString(key2), ":", value]);
        }
      });
...
(function(module, exports$1) {
  ...
  JavaScriptCompiler.prototype = {
    ...
    resolvePath: function resolvePath(type2, parts, startPartIndex, falsy, strict) {
      var _this2 = this;
      if (this.options.strict || this.options.assumeObjects) {
        this.push(strictLookup(this.options.strict && strict, this, parts, startPartIndex, type2));
        return;
      }
      var len = parts.length;
      var _loop = function(i2) {
        _this2.replaceStack(function(current) {
          var lookup2 = _this2.nameLookup(current, parts[i2], type2);
          if (!falsy) {
    ...
    assignToHash: function assignToHash(key2) {
      ...
      var hash = this.hash;
      if (context) {
        hash.contexts[key2] = context;
      }
      if (type2) {
        hash.types[key2] = type2;
      }
      if (id2) {
        hash.ids[key2] = id2;
      }
      hash.values[key2] = value;
    },
  ...
  function strictLookup(requireTerminal, compiler2, parts, startPartIndex, type2) {
    ...
    if (requireTerminal) {
      return [compiler2.aliasable("container.strict"), "(", stack, ", ", compiler2.quotedString(parts[len]), ", ", JSON.stringify(compiler2.source.currentLocation), " )"];
    } else {
      return stack;
    }
  }
...
PrintVisitor.prototype.Program = function(program) {
  var out2 = "", body = program.body, i = void 0, l = void 0;
  if (program.blockParams) {
    var blockParams2 = "BLOCK PARAMS: [";
    for (i = 0, l = program.blockParams.length; i < l; i++) {
      blockParams2 += " " + program.blockParams[i];
    }
    blockParams2 += " ]";
    out2 += this.pad(blockParams2);
  }
  for (i = 0, l = body.length; i < l; i++) {
    out2 += this.accept(body[i]);
  }
  this.padding--;
  return out2;
};
...
PrintVisitor.prototype.BlockStatement = PrintVisitor.prototype.DecoratorBlock = function(block) {
  var out2 = "";
  out2 += this.pad((block.type === "DecoratorBlock" ? "DIRECTIVE " : "") + "BLOCK:");
  this.padding++;
  out2 += this.pad(this.SubExpression(block));
  if (block.program) {
    out2 += this.pad("PROGRAM:");
    this.padding++;
    out2 += this.accept(block.program);
    this.padding--;
  }
  if (block.inverse) {
    if (block.program) {
    ...
    out2 += this.pad("{{^}}");
    this.padding++;
    out2 += this.accept(block.inverse);
    this.padding--;
    if (block.program) {
      this.padding--;
    }
...
PrintVisitor.prototype.PathExpression = function(id2) {
  var path2 = id2.parts.join("/");
  return (id2.data ? "@" : "") + "PATH:" + path2;
};
...
class PdfCreatorService {
  ...
  static getTemplateDir() {
    if (process.env.VITE_DEV_SERVER_URL) {
      return require$$1$1.join(process.cwd(), "public", "pdf_templates");
    } else {
      const resourcesBase = process.resourcesPath || require$$1$1.join(require$$1$1.dirname(app.getAppPath()), "..");
      return require$$1$1.join(resourcesBase, "public", "pdf_templates");
    }
  }
  /**
  ...
  static async loadTemplate(templateId) {
    ...
    const templatePath = require$$1$1.join(templateDir, `${templateId}.html`);
    if (!fs$j.existsSync(templatePath)) {
      throw new Error(`Sablon nem található: ${templateId}`);
    }
    const templateContent = fs$j.readFileSync(templatePath, "utf-8");
   ...
   */
  static async renderTemplate(template2, payload) {
    try {
      return template2(payload);
    } catch (error2) {
      throw new Error(`Sablon renderelés hiba: ${error2 instanceof Error ? error2.message : "Ismeretlen hiba"}`);
    }
  }
  /**
   ...
   */
  static async generatePdf(html, outputPath) {
    const outputDir = require$$1$1.dirname(outputPath);
    if (!fs$j.existsSync(outputDir)) {
      fs$j.mkdirSync(outputDir, { recursive: true });
    }
    ...
    try {
      await win2.webContents.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
      ...
      await fs$j.promises.writeFile(outputPath, pdfData);
      const pageCount = Math.max(1, Math.ceil(pdfData.length / 15e3));
      return { success: true, pageCount };
    } catch (error2) {
      throw new Error(`PDF generálás hiba: ${error2 instanceof Error ? error2.message : "Ismeretlen hiba"}`);
    } finally {
      if (!win2.isDestroyed()) {
        win2.destroy();
      }
   ...
   */
  static async saveAndOpen(filePath, autoOpen = true) {
    if (autoOpen) {
      try {
        const { default: open } = await import("./index-Dhr-x8Jx.js");
        void open(filePath).catch((error2) => {
          console.warn(
            `Nem sikerult megnyitni a PDF-et: ${error2 instanceof Error ? error2.message : "Ismeretlen hiba"}`
          );
        });
      } catch (error2) {
        console.warn(`Nem sikerült megnyitni a PDF-et: ${error2 instanceof Error ? error2.message : "Ismeretlen hiba"}`);
      }
    }
  }
  /**
   ...
   */
  static async createSchematicPdf(request) {
    try {
      const template2 = await this.loadTemplate(request.templateId);
      const html = await this.renderTemplate(template2, request.payload);
      ...
      return {
        success: false,
        error: error2 instanceof Error ? error2.message : "Ismeretlen hiba történt a PDF generálás során"
      };
    }
  }
}
...
function createWindow() {
  ...
  win.on("close", async (event) => {
    ...
    const result = await dialog.showMessageBox(win, {
      type: "question",
      buttons: isDownloadingNow ? ["Mégse", "Bezárás és letöltés megszakítása"] : ["Mégse", "Bezárás és frissítés telepítése"],
      defaultId: 1,
      cancelId: 0,
      ...
      detail: isDownloadingNow ? "Ha most bezárod az alkalmazást, a letöltés megszakad. Biztosan be szeretnéd zárni?" : "Az alkalmazás bezárásakor a frissítés települ. Biztosan be szeretnéd zárni most?",
      noLink: true
    });
...
ipcMain.handle("open-file", async (_event, filePath) => {
  const basePath = app.isPackaged ? path$m.join(process.resourcesPath, "public") : path$m.join(__dirname$1, "../public");
  openFile(path$m.join(basePath, filePath));
  return true;
});
...
ipcMain.handle("pdf-create-schematic", async (_event, request) => {
  try {
    ...
    console.error("PDF creation error:", error2);
    return {
      success: false,
      error: error2 instanceof Error ? error2.message : "PDF generálás hiba"
    };
  }
});
ipcMain.handle("pdf-generate", async (_event, companyId, templateId, payload) => {
  try {
    console.log("PDF generation request received:", { companyId, templateId });
    const timestamp2 = Date.now();
    const filename = `${companyId}-${templateId}-${timestamp2}.pdf`;
    const outputPath = path$m.join(
      app.isPackaged ? path$m.join(process.resourcesPath, "generated_pdfs") : path$m.join(process.cwd(), "generated_pdfs"),
      filename
    );
    const response = await PdfCreatorService.createSchematicPdf({
      companyId,
      templateId,
      payload,
      outputPath,
