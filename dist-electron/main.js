...
bt.handle("run-executable", (e, t) => new Promise((r, n) => {
  var f, c;
  const i = cr.isPackaged ? Te.join(process.resourcesPath, "public") : Te.join(ei, "../public"), a = Te.join(i, t);
  console.log("Running exe:", a);
  const o = wd(a, [], {
    windowsHide: !0,
    cwd: Te.dirname(a)
  });
  let s = "", l = "";
  (f = o.stdout) == null || f.on("data", (u) => {
    const d = rs.decode(u, "win1250");
    s += d, e.sender.send("exe-stdout", d);
  }), (c = o.stderr) == null || c.on("data", (u) => {
    const d = rs.decode(u, "win1250");
    l += d, e.sender.send("exe-stderr", d);
  }), o.on("close", (u) => {
    u === 0 ? r(s || "Executed successfully") : n(l || `Process exited with code ${u}`);
  }), o.on("error", (u) => n(u.message));
}));
